...
function findStudentSalary(){
	$('#table').bootstrapTable({
		url : "jobbingEmployeeServlet?method=findJobbingSalaryInfo",
		striped : true,
		pagination : true,
		clickToSelect : true,
...
function updateSalaryJobbingInfo(){
	bootbox.confirm({
	    title: "确认提示",
	    message: "是否真的修改所选外协员工薪资？",
	    buttons: {
	        cancel: {
	    ...
	    callback: function (result) {
	       if(result==true){
	    	   $("#updateOuterSalaryForm").ajaxSubmit({
	   			url : "jobbingEmployeeServlet?method=updateJobbingSalary",
	   			type : "post",
	   			dataType:"json",
	   			success : function(result) {
	   				if(result==true){
	   					$('#table').bootstrapTable("refresh");
	   					$("#tipmsg").css("display", "block");
		   				$("#tipmsg").html("修改外协员工薪资成功！");
	   				}else
	   					{
	   					$("#tipmsg").css("display", "block");
		   				$("#tipmsg").html("修改外协员工薪资失败，请重试！");
	   					}
    ...
    <!-- Content Header (Page header) -->
     <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-10">
           <h2>外协人员薪资管理 | Cooperator Salary Management</h2>
          </div>
          <!-- <div class="col-sm-6">
    ...
    <div class="modal-dialog">
		...
		<input type="text" id="normal_pafhc" name="normal_pafhc" class="form-control"/></div>				
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" onclick="updateSalaryJobbingInfo()">提交更改</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal -->
</div>
