...
function myFormatter(value, row, index) {
		var a = "<a href='javascript:updateEmp_studentByEmpno(\""+row.empno+"\")' title='编辑'><span class='btn btn-success'>修改</span></a>"
		var b = "<a href='javascript:deleteEmp_studentByEmpno(\"" + row.empno
	...
	function updateEmp_studentByEmpno(empno){
		//init();
	//alert("bbb");
		...
		$("#normal_school").val(result.school);
	...
	function updateStudentEmployeeInfoByempno(){
	//	alert("1111111");
		bootbox.confirm({
		    title: "确认提示",
		    message: "是否真的修改本行实习生信息？",
		    buttons: {
		        cancel: {
		    ...
		    callback: function (result) {
		       if(result==true){
		    	   $("#updateNormalEmployeeForm").ajaxSubmit({
		   			url : "studentEmployeeServlet?method=updateStudentEmployeeInfoByEmpno",
		   			type : "post",
		   			dataType:"json",
		   			success : function(result) {
		   				if(result==true){
		   					$('#example2').bootstrapTable("refresh");
		   					$("#tipmsg").css("display", "block");
			   				$("#tipmsg").html("修改实习生信息成功！");
		   				}else
		   					{
		   					$("#tipmsg").css("display", "block");
			   				$("#tipmsg").html("修改实习生信息失败，请重试！");
		   					}
	...
	function deleteEmp_studentByEmpno(empno) {
		//alert("sss");
		bootbox.confirm({
					message : "是否真的删除所选记录？",
					buttons : {
					...
					callback : function(result) {
						if (result == true) {
							$.ajax({
										url : "studentEmployeeServlet?method=deleteStudentEmployeeByEmpno",
										type : "post",
										data : {
											"empno" : empno
										},
	...
	function insertEmpByEmpno(){
		$("#insertStudentEmployee").modal("show");
	}
	function insertStudentEmployeeInfo(){
		bootbox.confirm({
		...
		callback: function (result) {
   		if(result==true){
	  		 $("#insertStudentEmployeeForm").ajaxSubmit({
			url : "studentEmployeeServlet?method=entryStudentEmpInfo",
			type : "post",
			dataType:"json",
			success : function(result) {
				if(result==true){
   				...
   				//window.location.href="searchStaffInfo.jsp";
					$("#insertStudentEmployee").modal("hide");
    ...
    <!-- Content Header (Page header) -->
     <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-8">
            <h2>实习生信息查询 | Trainee Information Service </h2>
          </div>
...
<div class="modal fade" id="normalEmployee" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog" align="center">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="updatenormalemployee">修改实习生信息</h4>
		...
		&nbsp;&nbsp;<div class="input-group form-inline col-sm-10 ">
		<span class="input-group-addon" style="background-color: #0099CC;color:white">所在学校</span>
		<input type="text" id="normal_school" name="normal_school" class="form-control"/></div>
		&nbsp;&nbsp;<div class="input-group form-inline col-sm-10 ">
		...
		&nbsp;&nbsp;<div class="input-group form-inline col-sm-10 ">
		<span class="input-group-addon" style="background-color: #0099CC;color:white">&nbsp;&nbsp;&nbsp;年级&nbsp;&nbsp;&nbsp;&nbsp;</span>
		...
		<input type="text" id="normal_email" name="normal_email" class="form-control"/></div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                <button type="button" class="btn btn-primary" onclick="updateStudentEmployeeInfoByempno()">确认修改</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
</div>
...
<!-- 添加员工信息模态框 -->
<div class="modal fade" id="insertStudentEmployee" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog" align="center">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="insertnormalemployee">添加实习生信息</h4>
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
            </div>
            <div class="modal-body">
              <div id="adddictypevaluetipmsg" class="alert alert-warning" style="display: none"></div>
              <form id="insertStudentEmployeeForm" action="#" method="post" class="form-horizontal">
		...
		<input type="text" id="normal_email" name="normal_email" class="form-control"/></div>		
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                <button type="button" class="btn btn-primary" onclick="insertStudentEmployeeInfo()">确认添加</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
</div>		
